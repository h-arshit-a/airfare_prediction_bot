...
export class AviationstackService {
    ...
    private constructor() {
        ...
        if (!this.apiKey) {
            console.warn("No AviationStack API key found in environment variables. Using mock responses.");
            this.useMocks = true;
        }
        
        console.log(`AviationStack service initialized with ${this.useMocks ? 'MOCK' : 'REAL'} responses`);
    }
    ...
    async getFlights(params: {
        ...
        try {
            ...
            if (this.useMocks) {
                console.log('Using mock flight data instead of API');
            ...
            const response = await fetch(`${API_BASE_URLS.AVIATIONSTACK}${API_ENDPOINTS.AVIATIONSTACK.FLIGHTS}?${queryParams}`);
    ...
    async getAirports(params: {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URLS.AVIATIONSTACK}${API_ENDPOINTS.AVIATIONSTACK.AIRPORTS}?${queryParams}`);
    ...
    async getAirlines(params: {
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URLS.AVIATIONSTACK}${API_ENDPOINTS.AVIATIONSTACK.AIRLINES}?${queryParams}`);
    ...
    private getMockFlights(params: any): FlightResponse {
        ...
        const departure = params.departure_iata || cities[Math.floor(Math.random() * cities.length)];
        const arrival = params.arrival_iata || cities[Math.floor(Math.random() * cities.length)];
        ...
        for (let i = 0; i < count; i++) {
            ...
            const arrivalTime = new Date(departureTime.getTime() + (Math.random() * 5 * 60 * 60 * 1000));
            ...
            flightData.push({
                ...
                airline: airline,
                aircraft: {
                    registration: `VT-${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}`,
                    iata: 'A320',
                    icao: 'A320',
                    model: 'Airbus A320'
                },
    ...
    private getMockAirports(params: any): AirportResponse {
        ...
        if (params.country_iso2) {
            filteredAirports = airports.filter(airport => airport.country_iso2 === params.country_iso2);
        }
        
        // Filter by city if provided
        if (params.city_iata_code) {
            filteredAirports = filteredAirports.filter(airport => airport.city_iata_code === params.city_iata_code);
        }
    ...
    private getMockAirlines(params: any): AirlineResponse {
        ...
        if (params.country_iso2) {
            filteredAirlines = filteredAirlines.filter(airline => airline.status === 'active');
        }
