...
const FlightResultsList: React.FC<FlightResultsListProps> = ({ flights, searchDate, source, destination }) => {
  if (flights.length === 0) {
    return null;
  }

  // Get cheapest flight
  const cheapestFlight = flights.reduce((prev, current) => (prev.price < current.price) ? prev : current);
  ...
  return (
    <div className="mt-6 bg-white rounded-lg shadow-md overflow-hidden">
      <div className="bg-background p-4 border-b">
        <h2 className="text-xl font-bold">Flights from {source} to {destination}</h2>
      </div>
          ...
          {dates.map((date, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={cn("mt-1", isSelected ? "text-primary font-bold" : "")}>
      ...
      <Tabs defaultValue="cheapest" className="w-full">
        <TabsList className="w-full justify-start p-0 border-b bg-white rounded-none h-auto">
          ...
          >
            <span className="rounded-md bg-blue-100 p-2 mr-2 text-primary">₹</span>
            CHEAPEST
            <span className="text-sm text-muted-foreground ml-2">₹{cheapestFlight.price} | {format(new Date(cheapestFlight.departure_time), "HH:mm")} - {format(new Date(cheapestFlight.arrival_time), "HH:mm")}</span>
          </TabsTrigger>
          <TabsTrigger 
            value="fastest" 
            className="data-[state=active]:bg-blue-50 data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:rounded-none py-3 px-4"
        ...
        <div className="p-2 text-sm text-muted-foreground bg-muted/20 border-b">
          Flights sorted by Lowest fares on this route
        </div>

        <TabsContent value="cheapest" className="mt-0">
          {flights.map((flight) => (
            <div key={flight.id} className="border-b last:border-b-0 p-4 hover:bg-muted/10">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                ...
                <div className="flex items-center w-full md:w-1/4">
                  <div className="w-10 h-10 rounded bg-muted flex items-center justify-center mr-3">
                    <span className="font-bold text-xs">{flight.airline.substring(0, 2)}</span>
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-between w-full md:w-2/5">
                  ...
                  <div className="flex flex-col items-center mx-4">
                    <div className="text-xs text-muted-foreground mb-1 flex items-center">
                      ...
                      {Math.floor((new Date(flight.arrival_time).getTime() - new Date(flight.departure_time).getTime()) / (1000 * 60))} min
                    </div>
                ...
                <div className="flex flex-col items-end w-full md:w-1/4">
                  <div className="text-xl font-bold text-right">₹{flight.price}</div>
                  ...
                  {Math.random() > 0.5 && (
                    <div className="flex items-center text-xs text-blue-600 mt-2">
                      ...
                      <span>Lock this price starting from ₹{Math.floor(flight.price * 0.05)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {Math.random() > 0.3 && (
                <div className="mt-3 bg-orange-50 text-orange-700 rounded-sm p-2 text-xs flex items-center">
                  ...
                  <span>Get ₹{Math.floor(Math.random() * 300)} off using MMTPROMO</span>
                </div>
              )}
              
              {/* Details link */}
              <div className="mt-3 text-right">
                <Button variant="link" className="text-sm p-0">View Flight Details</Button>
              </div>
            </div>
          ))}
        </TabsContent>
        ...
        <TabsContent value="fastest" className="mt-0">
          <div className="p-4 text-center">
            <p>Fastest flight options will appear here.</p>
          </div>
        </TabsContent>
        
        <TabsContent value="best" className="mt-0">
          <div className="p-4 text-center">
            <p>Best value flight options will appear here.</p>
          </div>
        </TabsContent>
      </Tabs>
    </div>
