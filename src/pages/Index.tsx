...
import { generateChatbotResponse, getInitialMessages, parseFlightSearchCommand } from '@/services/chatbotService';
import { searchFlights, getFlightDeals, Flight, FlightSearchParams, FlightDeal } from '@/services/flightService';
...
const Index = () => {
  ...
  const handleSendMessage = async (messageText: string) => {
    ...
    try {
      ...
      const response = await generateChatbotResponse(messageText, searchParams, flights);
  ...
  const handleFlightSearch = async (data: FlightSearchParams) => {
    ...
    try {
      // Search for flights
      console.log(`Searching flights from ${data.source} to ${data.destination} on ${data.date.toDateString()}`);
      const flightResults = await searchFlights(data);
      console.log(`Found ${flightResults.length} flights, first flight:`, flightResults.length > 0 ? flightResults[0] : 'none');
      ...
      console.log(`Response length: ${response.length}, contains flight-results: ${response.includes('<flight-results>')}`);
      ...
      addMessage('Sorry, I encountered an error while searching for flights. Please try again.', 'bot');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="chat-header">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Ask me about flights, travel tips, and more</p>
              </div>
            </div>
          </div>
