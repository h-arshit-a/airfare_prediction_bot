...
import { Plane, MapPin, Clock, MessageSquareText, Info, Zap, Sun, Moon } from "lucide-react";
...
const Welcome = () => {
  ...
  useEffect(() => {
    setDynamicMessage(welcomeMessages[Math.floor(Math.random() * welcomeMessages.length)]);
    setDynamicFact(travelFacts[Math.floor(Math.random() * travelFacts.length)]);
  }, []);

  const features = [
    { icon: <Plane className="h-8 w-8 text-blue-500" />, title: "Flight Information", description: "Real-time updates on statuses, delays, and gates." },
    { icon: <MapPin className="h-8 w-8 text-blue-500" />, title: "Airport Details", description: "Info on terminals, amenities, and transport." },
    { icon: <Clock className="h-8 w-8 text-blue-500" />, title: "Travel Planning", description: "Assistance with connections and layovers." },
    { icon: <Zap className="h-8 w-8 text-blue-500" />, title: "Instant Answers", description: "Quick replies to your travel questions." }
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h1 className="text-5xl md:text-7xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-teal-500 dark:from-blue-400 dark:to-teal-300 mb-4">
        ...
        >
          {features.map((feature, index) => (
            ...
            >
              <div className="flex-shrink-0 mt-1 p-2 bg-blue-100 dark:bg-blue-900/30 rounded-full">{feature.icon}</div>
              <div>
                <h3 className="font-semibold text-md text-blue-700 dark:text-blue-300">{feature.title}</h3>
